...
class moodle_crosslist_testcase extends xml_helper {

    public function test_crosslist_missing_children() {
        global $CFG, $DB;
        $this->resetAfterTest(true);
        $unkcatid = $this->setup_unknown_category();

        // First we are going to do something silly - renumber the starting category so that it doesn't start at 1...
        $DB->set_field('course_categories', 'id', 1000, ['id' => 1]);
        settings_helper::temp_set('catselect', 1000);
        ...
        $moodlecrosslist = new moodle\crosslist();

        // We haven't loaded in the child courses, so this isn't going to know what to do.
        $moodlecrosslist->convert_to_moodle($crosslist);
